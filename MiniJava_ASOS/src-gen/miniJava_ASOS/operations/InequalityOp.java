...
import miniJava_ASOS.operations.data.InequalityData;
...
@Operationalize(node = Inequality.class, visitor = "miniJava_ASOS.interpreter.MiniJava_ASOSVisitor")
public class InequalityOp extends AdaptiveOperation<Inequality, miniJava_ASOSInterface>{
	
	@Override
	public Object execute(SelfAdaptiveVisitor vis, Inequality node, Object execCtxObj, miniJava_ASOSInterface config) {
		Object result = null;
		State execCtx = ((State) execCtxObj);
		
		InequalityData data = new InequalityData(node);
		
		while(true){
			Object termination = null;
			InequalityData local_data = new InequalityData(node);
		
			boolean validPatternInput95 = true;
			if(validPatternInput95)
			{
			...
			boolean validPatternInput96 = true && (data.getcomputed_left() instanceof Value);
			if(validPatternInput96)
			{
			...
			boolean validPatternInput97 = true && (data.getcomputed_left() instanceof BooleanValue) && (data.getcomputed_right() instanceof BooleanValue);
			if(validPatternInput97)
			{
				if(config.before_inequal() != null){
					config.before_inequal().adapt(vis, node, execCtx, data, config);
				}
				
				if(config.specialize_inequal() != null){
					config.specialize_inequal().adapt(vis, node, execCtx, data, config);
				} else {
					Object n = (!((BooleanValue) data.getcomputed_left()).getValue().equals(((BooleanValue) data.getcomputed_right()).getValue()));
					BooleanValue booleanValue9 = SdFactory.eINSTANCE.createBooleanValue();
					booleanValue9.setValue((java.lang.Boolean) n);
					BooleanValue out = booleanValue9;
					result = out;
				}
				
				if(config.after_inequal() != null){
					if(result == null){
						config.after_inequal().adapt(vis, node, execCtx, data, config);
					} else {
						config.after_inequal().adapt(vis, ((AdaptableNode<miniJava_ASOSInterface>) result), execCtx, null, config);
					}
				}
