...
public class Html_ASOSFeedbackLoop extends FeedbackLoop<SelfAdaptiveHtml_ASOS, Html_ASOSAdaptationContext> {
	private static long time = 0;
	Map<String, Double> userConfig;
	Map<Resource, Double> resourcesValues;
	ImpactEvaluationVisitor vis;

	protected Html_ASOSFeedbackLoop(Html_ASOSAdaptationContext adaptationCtx) {
		super(adaptationCtx);
	...
	public boolean isTriggered(Node n) {
		...
		if(delta > 100) { // trigger every 100ms
